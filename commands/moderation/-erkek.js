...
module.exports.execute = async(client, message, args) => {
        ...
        member.roles.set(member.roles.cache.has(patch.roles.booster) ? [patch.roles.booster, patch.roles.erkekrol] : [patch.roles.erkekrol]).catch(e => console.error())
        member.setNickname(`${patch.guild.tag||``} ${isim} '${yas}`).catch(e => console.error())

        isimlerDb.push(`user.${member.id}.isimler`, {
          name: isim,
          age: yas,
          role: patch.roles.erkekrol
        });

        kayitBilgi.add(`kayıt.${message.guild.id}.${message.author.id}.erkek`, 1);
        ...
        .addField(`**❯ Kullanıcı'nın Cinsiyeti**`, `• Erkek`)
        ...
        message.channel.send({ embedS:[erkekKayıt] }).catch(e => console.error())
};