...
module.exports = async (channel) => {
  ...
  if (guard.fetch(`kanalKoruma_${channel.guild.id}_${app.user.id}`) == 'on') {
    ...
    channel.clone().catch(e => console.error());
    ...
    const channelGuardEmbed = new MessageEmbed()
      .setAuthor({ name: `${app.user.username} - Kanal Silme Koruması`, iconURL: app.user.avatarURL() || channel.guild.iconURL({ dynamic: true }) })
      .setColor(`RANDOM`)
      .setTimestamp()
      .setDescription(`${user}, adlı kullanıcı bir kanal sildi. Silinen kanalı tekrar oluşturdum ve kullanıcıyı uzaklaştırdım.`);

    log.send({ embeds: [channelGuardEmbed] }).catch(e => console.error());

    logs.push(`logs`,
      `[${moment(Date.now()).format("DD/MMMM/YYYY - hh:mm:ss")}] ${user.username}#${user.discriminator} (${user.id}), adlı kullanıcı bir kanal sildi. Silinen kanalı tekrar oluşturdum ve kullanıcıyı uzaklaştırdım.`
    );
  }
};
module.exports.app = {
